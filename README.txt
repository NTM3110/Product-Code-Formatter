Product Code Formatter - MST All Names V6

New in V6:
- The company check page shows all company names found under the same MST.
- The app still groups by MST người bán.
- The main name used for prefix suggestion is the most common company name for that MST.
- You can still tick/untick which MST/company group to process.

Previous features:
- User can choose columns in the UI.
- Prefix suggestion uses the company name: first character of the last 2 words.
- If generated prefix duplicates another selected MST, user must enter manually.
- Sheet1 is not required.
- Rows where quantity is empty, invalid, or 0 are skipped.
- Output workbook includes CompanyPrefixes sheet.

Build:
1. Extract ZIP.
2. Double-click build_exe_auto.bat.
3. Run dist\ProductCodeFormatter.exe.
